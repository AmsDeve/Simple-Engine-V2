...
hudGroup.add(scoreTxt);
		...
		FlxG.camera.zoom = 3;
		FlxTween.tween(FlxG.camera, {zoom: 1}, 1.1, {ease: FlxEase.sineInOut});
	...
	override public function update(elapsed:Float)
	{
		...
		scoreTxt.text = 'Score: ${songScore} | Rating: ${rating} | Misses: ${missesCounter} | GoodHits: ${goodHitsCounter} | Combo: ${comboPercentaje}%';
		scoreTxt.screenCenter(X);

		if (healthBar.percent > 22)
			healthTxt.color = FlxColor.WHITE;
		else if (healthBar.percent > 20)
			healthTxt.color = FlxColor.RED;

		if (comboPercentaje > 100)
			comboPercentaje = 100;

		if (songScore == 0)
			rating = "?"; 

		if (goodHitsCounter == 1)
			rating = "GFC"; 
	...
	private function popUpScore(strumtime:Float):Void
	{
		...
		for (i in seperatedScore)
		{
			...
			numScore.x = comboSpr.x + (43 * daLoop) - 120;
			numScore.y += comboSpr.y - 257;
			if(curStage.startsWith('school'))
				{
					numScore.y += comboSpr.y - 410;
					numScore.x = comboSpr.x + (43 * daLoop) - 140;
				}
...
rating.cameras = [camHUD];
					...
					if (possibleNotes[0].strumTime == possibleNotes[1].strumTime)
					{
						for (coolNote in possibleNotes)
						{
							...
							else
							{
								...
								if (!inIgnoreList)
									(FlxG.save.data.ghostTapping ? "" : badNoteCheck());
							}
						}
					}
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'blammed' && curBeat >= 154 && curBeat < 159)
			{
				FlxTween.tween(FlxG.camera, {zoom: 1.5}, (Conductor.stepCrochet * 4 / 1000), {ease: FlxEase.elasticInOut});
			}
